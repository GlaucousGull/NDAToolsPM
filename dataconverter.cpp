...
DataConverter::ConversionResult DataConverter::decimalToBinary(const QString& decimalString)
{
    ...
    if(decimalString.isEmpty()) {
        return ConversionResult(false, "", "输入的十进制为空");
    }

    // 数值解析-将字符串转换为qint64整数
    bool ok;
    ...
    if(decimalValue == 0) {
        ...
        // 修复：类型匹配，quint32接收quint32
        quint32 unsignedValue = static_cast<quint32>(decimalValue);
        result = QString::number(unsignedValue, 2).rightJustified(32, '0');
    }

    // 格式化输出-按8位一组用空格输出（核心修复）
    QString formattedResult;
    // 预分配空间：结果长度 + 空格数（每8位一个空格，向上取整）
    formattedResult.reserve(result.length() + (result.length() + 7) / 8);

    // 正确分组逻辑：从后往前每8位一组，保证高位在前
    int len = result.length();
    for(int i = len; i > 0; i -= 8) {
        int start = qMax(0, i - 8);
        if(!formattedResult.isEmpty()) {
            formattedResult.prepend(' ');
        }
        formattedResult.prepend(result.mid(start, i - start));
    }
...
DataConverter::ConversionResult DataConverter::binaryToDecimal(const QString& binaryString)
{
    ...
    // 长度限制-最多只能支持63位(qint64的最大位数）
    if(cleanBinary.length() > 63) {
        return ConversionResult(false, "", "二进制字符串超限，最多支持63位");
    }

    // 数值转换-将二进制字符串转换位qint64整数
    bool ok;
    qint64 decimalValue = cleanBinary.toLongLong(&ok, 2);

    if(!ok) {
        return ConversionResult(false, "", "二进制转换失败，数值可能超出有效范围");
    }
...
{
    // 获取字符数（Unicode字符个数）
    int countChar = countCharacter(data);

    // 获取字节数（UTF-8编码后的字节数）
    int countByte = countBytes(data);
    return QStringLiteral("字符数：%1，字节数：%2").arg(countChar).arg(countByte);
}

// 计算所有常用校验值
DataConverter::CheckSumResult  DataConverter::calculateCheckSums(const QString& data)
{
    // 将字符串转换为 UTF-8编码的字节数组
    QByteArray bytes = data.toUtf8();

    // 调用字节数组版本的函数进行计算
    return calculateCheckSums(bytes);
}

// 计算所有常用校验值(字节数组输入版本重载)
// 一次性计算CRC16、CRC32、MD5、SHA1、SHA256五种校验值
// 所有校验值都以十六进制字符串实行返回
// 计算CRC16：使用查找表算法，记过格式化为4位十六机制（高位补零）
// 计算CRC32：使用查找表算法，记过格式化为8位十六机制（高位补零）
// 计算MD5：使用Qt的QCryptographicHash，结果格式化为32位十六进制
// 计算SHA1：使用Qt的QCryptographicHash，结果格式化为40位十六进制
// 计算SHA256：使用Qt的QCryptographicHash，结果格式化为64位十六进制
DataConverter::CheckSumResult  DataConverter::calculateCheckSums(const QByteArray& data)
{
    struct CheckSumResult result;

    result.crc16 = QString::number(calculateCRC16(data), 16).toUpper().rightJustified(4, '0');
    result.crc32 = QString::number(calculateCRC32(data), 16).toUpper().rightJustified(4, '0');
...
quint16 DataConverter::calculateCRC16(const QByteArray& data)
{
    ...
    for(int i = 0; i < data.size(); i++) {
        ...
        // 查找表算法
        crc = static_cast<quint16>((crc << 8) ^ crc16Table[((crc >> 8) ^ byte) & 0xFF]);
    }
    return crc;
}
...
quint32 DataConverter::calculateCRC32(const QByteArray& data)
{
    ...
    for(int i = 0; i < data.size(); i++) {
        ...
        // 查找表算法
        crc = static_cast<quint32>((crc >> 8) ^ crc32Table[(crc & 0xFF) ^ byte]);
    }
